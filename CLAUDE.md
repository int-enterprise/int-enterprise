...
### 히어로 — 첫 화면을 채우는 핀 구간 + 가운데 3D
랜딩 히어로는 **좌(워드마크·브랜드 뜻·버튼) / 중앙(3D 오브젝트) / 우(비움)**이다.
흰 여백을 넓게 두고 활자는 조용하게, 시각 요소는 가운데 3D 하나로 몬다.
**우측 칸은 3D를 화면 가운데에 두려고 비워 둔 것이다** — 슬로건 문단을 다시 채우지 않는다.

- **스크롤 연출**: 바깥 섹션이 200svh 활주로, 안쪽이 `sticky top-0 h-[100svh]`로 핀된다(`hero-stage.tsx`).
  한 화면을 내리는 동안 히어로가 화면에 붙어 있고 3D가 천천히 회전하며 문구가 흐려진다. 활주로가 끝나면 핀이 풀린다.
  진행도(0~1)는 CSS 변수 `--hero-p`와 `HeroProgressContext`의 ref 두 갈래로 흐른다 — **state로 바꾸지 않는다**(프레임마다 리렌더된다).
- 헤더가 흐름에서 차지하는 64px은 섹션의 `-mt-16`으로 상쇄한다. 빼면 첫 화면 아래가 잘린다.
- 스크롤 진행에 따른 3D 변화는 **회전 위주**로 만든다. 크게 키우거나 위로 많이 올리면 도형이 캔버스 밖으로 잘려 깨진 화면이 된다.
